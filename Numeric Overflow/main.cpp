...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if ((increment > 0) && (result > std::numeric_limits<T>::max() - increment)) { // Added if statement to check if increments are > 0 and if max num limits - increment.
...
void test_overflow()
{
    ...
    T result2 = add_numbers<T>(start, increment, steps + 1) ;  // Modified to declare result2 variable to bounce off result variable.

    if (result2 - std::numeric_limits<T>::min() == result) {  // Check if result2 == result.
        std::cout << +(result2 - std::numeric_limits<T>::min()) << std::endl;  // Output result2 - min num limit.
        std::cout << "\tOverflow Occurred" << std::endl; // Output if overflow occurred.
    }
    else {  // Else statement to declare no overflow occurred if that is the case.

        std::cout << +(result2) << std::endl;
        std::cout << "\tNo Overflow Occurred" << std::endl;
    }
...
void test_underflow()
{
    ...
    if (result2 == std::numeric_limits<T>::max()) {  // Check if result2 == max num limits.
        std::cout << +(result) << std::endl;  // Output result.
        std::cout << "\tUnderflow Ocurred" << std::endl;  // Declare occurrence of underflow.
    }
// Else statement to output result2 and no overflow occurred.
    else {
        std::cout << +(result2) << std::endl;
